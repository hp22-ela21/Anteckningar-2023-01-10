  ...
  ********************************************************************************/
   ...
   ********************************************************************************/
   void shuffle(void)
   {
      for (std::size_t i = 0; i < this->num_training_sets(); ++i)
      {
         const auto r = std::rand() % this->num_training_sets();
   ...
   ********************************************************************************/
   void optimize(const double input,
                 ...
                 const double learning_rate)
   {
      const auto error = reference - this->predict(input);
      this->bias += error * learning_rate;
      this->weight += error * learning_rate * input;
      return;
   }
};
